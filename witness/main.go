...
// trustedOrigin holds a trusted origin's verifier key. The parsed form serves
// git-checkpoint mode; tlog mode passes the vkey to the formats package, which
// builds the verifier the key's algorithm calls for.
...
func parseOrigins(origins []string) (map[string]trustedOrigin, error) {
	...
	for _, vkey := range origins {
		...
		res[name] = trustedOrigin{vkey: vkey, pub: pub, sigType: sigType}
	}
	return res, nil
}
