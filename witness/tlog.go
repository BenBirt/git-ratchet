...
// cosigned to the tree it is asked to cosign. It never sees the entries, so it
// makes no statement about what they say.
func (s *Server) handleAddCheckpointTlog(w http.ResponseWriter, r *http.Request) {
	...
	_, sigLines, err := note.ParseSignedNote(req.Note)
	if err != nil {
	...
	verifier, err := fnote.NewVerifier(origin.vkey)
	if err != nil {
		log.Printf("unusable vkey for trusted origin %s: %v", originSigName, err)
		http.Error(w, "internal server error: unusable origin key", http.StatusInternalServerError)
		return
	}
	// ParseCheckpoint verifies the log's signature, parses the body, and
	// requires the origin line to match the log's key name. That last check is
	// what stops any origin the witness trusts from advancing another origin's
	// stored state.
	cp, _, _, err := flog.ParseCheckpoint([]byte(req.Note), originSigName, verifier)
	if err != nil {
		http.Error(w, fmt.Sprintf("invalid checkpoint: %v", err), http.StatusForbidden)
		return
	}
	cpRoot, err := tlog.RootFromBytes(cp.Hash)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	...
	if initialised {
		...
		if err := tlog.VerifyConsistency(storedRoot, cpRoot, req.Proof, stored.Size, cp.Size); err != nil {
	...
	if !initialised || cp.Size > stored.Size {
		s.trees[cp.Origin] = treeState{Size: cp.Size, Root: base64.StdEncoding.EncodeToString(cpRoot[:])}
