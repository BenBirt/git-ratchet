...
func (c *checkpointCmd) Execute(_ context.Context, f *flag.FlagSet, _ ...any) subcommands.ExitStatus {
	...
	if c.mode == modeTlog {
		...
		if err := checkpointTlog(c.repoDir, c.ref, origin, signer, tpol); err != nil {
			fmt.Fprintf(os.Stderr, "error: %v\n", err)
			return subcommands.ExitFailure
		}
...
func verifyRefs(repoDir string, refs []string, policyPath string, mode string) []verifyResult {
	...
	if mode == modeTlog {
		v, err := tlogViewFromPolicy(repoDir, policyPath)
		if err != nil {
		...
		for i, ref := range refs {
			results[i] = verifyResult{ref, verifySingleRefTlog(repoDir, ref, v)}
		}
		return results
	}
...
// tlogViewFromPolicy loads a tlog-policy and returns the checkpointed view of
// the repository's log under it.
func tlogViewFromPolicy(repoDir, policyPath string) (*gitlog.View, error) {
	tpol, err := policy.FromPath(policyPath)
	if err != nil {
		return nil, fmt.Errorf("loading policy: %w", err)
	}
	return checkpointedView(repoDir, tpol)
}
