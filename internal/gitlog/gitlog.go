...
//
// Hash tiles are not stored. The tree is recomputed from the entries, which
// every consumer holds: the log arrives with the repository.
package gitlog
...
func readEntries(repoDir string) ([]Entry, error) {
	out, err := gitutil.Git(repoDir).Run("ls-tree", "-r", "--name-only", LogRef, entriesPrefix)
	if err != nil {
...
func (l *Log) Size() uint64 { return tlog.Count(len(l.entries)) }
...
func (l *Log) ConsistencyProofFrom(m uint64) ([]tlog.Hash, error) {
...
func (l *Log) Save(checkpoint, message string) error {
	...
	if _, err := gitutil.Git(l.repoDir).Run("update-ref", LogRef, commit, l.head); err != nil {
...
func (l *Log) writeTree(blobs map[string]string) (string, error) {
	...
	for path, blob := range blobs {
		if _, err := gitutil.Git(l.repoDir).WithEnv(env...).Run(
			"update-index", "--add", "--cacheinfo", "100644,"+blob+","+path); err != nil {
			return "", fmt.Errorf("adding %s to log tree: %w", path, err)
		}
	}

	tree, err := gitutil.Git(l.repoDir).WithEnv(env...).Run("write-tree")
	if err != nil {
...
func (l *Log) commitTree(tree, message string) (string, error) {
	...
	commit, err := gitutil.Git(l.repoDir).WithEnv(env...).Run(args...)
	if err != nil {
