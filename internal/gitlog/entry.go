...
//
// The type identifier carries its own version; the framing has none. Entries
// are opaque byte strings to the log — bundles length-prefix them — so a type
// may use as many lines as it needs, newlines included.
//
// An entry whose type this implementation does not recognise still parses and
// still hashes; only its meaning is unavailable. See [ParseEntry].
const (
...
// pointed at when the entry was appended. It records state, not a transition —
// there is no predecessor field, and the log's ordering supplies one.
